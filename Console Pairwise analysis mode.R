...
data<-list.files()
the_data<- ld_undated_chron(files = data, series_names = NULL, pair_detrend = TRUE, detrending_select = 3, splinewindow = 21, powerT = FALSE, shiny = FALSE)
...
# view the best matches between the undated samples
print(as.data.frame(run_pairwise[1]))

# plot two samples and lag sample two.
line_plot(the_data = the_data, series_1 = colnames(the_data)[2], series_2 = colnames(the_data)[3], lag = 0)
